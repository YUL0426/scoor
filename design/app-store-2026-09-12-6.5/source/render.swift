...
// swift -module-cache-path /tmp/scoor-artwork-module-cache design/app-store-2026-09-12-6.5/source/render.swift

let root = URL(fileURLWithPath: FileManager.default.currentDirectoryPath)
let output = root.appendingPathComponent("design/app-store-2026-09-12-6.5")
let fm = FileManager.default
// One device at a time, to keep the local workload small.
let arguments = Array(CommandLine.arguments.dropFirst())
let devices = [arguments.first ?? "iphone-6.5"]
...
precondition(devices == ["iphone-6.5"], "This revision is the iPhone gallery")
...
final class Canvas {
    ...
    init(width: Int, height: Int, exportWidth: Int? = nil, exportHeight: Int? = nil) {
        self.width = CGFloat(width); self.height = CGFloat(height)
        let pixelWidth = exportWidth ?? width
        let pixelHeight = exportHeight ?? height
        context = CGContext(data:nil,width:pixelWidth,height:pixelHeight,bitsPerComponent:8,bytesPerRow:pixelWidth*4,
                            space:CGColorSpace(name:CGColorSpace.sRGB)!,
                            bitmapInfo:CGImageAlphaInfo.noneSkipLast.rawValue)!
        let scale = min(CGFloat(pixelWidth)/self.width, CGFloat(pixelHeight)/self.height)
        context.translateBy(x:(CGFloat(pixelWidth)-self.width*scale)/2,
                            y:(CGFloat(pixelHeight)-self.height*scale)/2)
    ...
    func fill(_ c: NSColor) { c.setFill(); NSRect(x:-10,y:-10,width:width+20,height:height+20).fill() }
...
for device in devices {
    ...
    for (index, copy) in pages.enumerated() {
        let canvas = Canvas(width:W,height:H,exportWidth:1284,exportHeight:2778)
        ...
        let raw = root.appendingPathComponent("design/app-store-2026-09-11/raw/\(locale)/iphone-6.9/\(copy.slug).png")
        ...
        print("Rendered \(device)/\(copy.slug).png 1284×2778")
    }
}
...
for device in devices {
    ...
    let thumbH = thumbW * (isPad ? 2752/2064.0 : 2778/1284.0)
    ...
    overview.text(isKorean ? "iPhone 6.5″ · 한국어" : "iPhone 6.5″ · English",x:48,y:65,size:18,color:color("66615D"),weight:.regular,tracking:0)
