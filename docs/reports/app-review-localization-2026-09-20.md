...
- 운영 서버 조회 결과 공개 토픽 5개와 초안 1개의 제목·설명은 한국어다. 이는 UI 문자열과 별개인 운영 콘텐츠다. 서버 데이터는 변경하지 않았다.
...
- 서버의 기본 점수 설명 `부정적/긍정적`, `반대/찬성`은 화면에서 현지화한다. 토픽 제목, 사용자 글, 사용자 정의 점수 설명은 원문을 유지한다.
- 문자열 카탈로그뿐 아니라 실제 컴파일된 영어 번들의 한국어 잔존 여부를 검사하는 회귀 테스트를 추가했다.
- 영어 가입·약관/개인정보 안내·개인 기록 저장·재실행 복원·설정·World·점수 입력 화면 검사를 추가했다.
- 재제출용 빌드 번호를 5로 올렸다. iPhone 전용 설정과 빨간 시작 화면은 유지한다.
...
이 아카이브는 Apple Development 서명 상태다. Xcode Organizer의 Distribute App → App Store Connect 단계에서 배포 처리해야 한다. 업로드나 심사 제출은 실행하지 않았다.
...
“iPhone 전용이므로 iPad 심사는 불필요하다”는 답변으로 문제를 해결할 수 없다. 영어 UI 누락을 수정한 새 빌드를 연결하고, 한국어 토픽·사용자 작성 콘텐츠가 원문으로 표시되는 의도된 동작임을 구분해 설명해야 한다. 특히 현재 공개 토픽 5개는 모두 한국어이므로 해외 사용자가 이해하기 쉬운 토픽 콘텐츠를 제공할지는 별도 제품 결정이다.
...
>
> Topic titles, descriptions, and user-written posts are shown in their original language and may contain Korean. Our current published topics are in Korean; this content behavior is intentional, while the mixed-language interface was not.
>
