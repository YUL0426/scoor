...
## 15. 확정된 결정 (2026-07-17)

착수 전 미결이던 5건은 모두 확정되었다. 다시 묻지 말고 아래를 전제로 진행한다.

| # | 항목 | 결정 | 설계에 미치는 영향 |
|---|---|---|---|
| 1 | 게스트 모드 | **허용** | 로그인 없이 개인 저널링 가능. 게시/좋아요/댓글/팔로우 시점에 로그인 요구 → 게스트가 쌓은 로컬 기록을 계정이 인수해야 한다(§7, `AccountMigrator`) |
| 2 | 피드 기본 노출 | **닉네임** | 게시 단위 익명 토글 제공. **"실명" 프레이밍은 카피/UI에서 쓰지 않는다** — 부담감을 준다는 이유로 명시적으로 배제된 표현이다 |
| 3 | 클로즈드 베타 | **초대 코드 300명**, 국내 우선 | 모더레이션 부하를 1인이 감당할 수 있는 상한 (§14) |
| 4 | World 토픽 운영 | **어드민 큐레이션 일 3~5개** | 어드민 토픽 큐레이션 화면이 Phase 1 필수 항목이 된다. `/admin/topics`가 live 3개 미만이면 경고한다 |
| 5 | 삭제 계정의 게시물 | **완전 삭제** (익명화 유지 아님) | 스키마를 CASCADE로 단순화. `account-delete`가 `auth.users` 삭제 → 전 테이블 연쇄 삭제 |
