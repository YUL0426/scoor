    ...
    <div className="flex-1 overflow-y-auto p-6 space-y-6">
      ...
      <div className="grid grid-cols-3 gap-4">
        {[
          ...
          <div
            key={m.label}
            className="bg-[#191a1d] border border-white/6 rounded-xl p-5"
          >
            <p className="text-xs text-[#8b8e98] uppercase tracking-wider mb-2">
              {m.label}
            </p>
            <p className="text-3xl font-bold text-[#ededee] mb-1 tabular-nums">
              {m.value}
            </p>
            <span
              className={`text-xs font-medium ${m.positive ? "text-emerald-400" : "text-red-400"}`}
            >
              전월 대비 {m.delta}
            </span>
          </div>
      ...
      {/* Scoor Trend */}
      <div className="bg-[#191a1d] border border-white/6 rounded-xl p-5">
        <div className="flex items-center justify-between mb-4">
          ...
          <div className="flex gap-2 text-xs">
            <span className="flex items-center gap-1.5 text-[#a2a4ac]">
              <span className="w-4 h-0.5 bg-[#e36b59] inline-block" />
              플랫폼
            </span>
            <span className="flex items-center gap-1.5 text-[#a2a4ac]">
      ...
      {/* Retention */}
      <div className="bg-[#191a1d] border border-white/6 rounded-xl p-5">
        <h3 className="text-sm font-semibold text-[#ededee] mb-4">
          사용자 리텐션 코호트 (%)
        </h3>
        <ResponsiveContainer width="100%" height={200}>
          <BarChart data={RETENTION_DATA}>
            ...
            <Legend
              iconType="square"
              iconSize={8}
              wrapperStyle={{ fontSize: 11, color: "#a2a4ac" }}
            />
            <Bar
              dataKey="d1"
              name="D1"
