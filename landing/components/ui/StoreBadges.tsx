...
export function AppStoreBadge({ className }: { className?: string }) {
  return (
    <a
      ...
      aria-label="Download Scoor on the App Store"
      className={cn(
        "inline-flex h-14 items-center gap-3 rounded-2xl bg-[var(--color-ink)] px-5 text-white transition-transform duration-300 hover:-translate-y-0.5",
        className
      )}
    >
...
export function PlayStoreBadge({ className }: { className?: string }) {
  return (
    <a
      ...
      aria-label="Get Scoor on Google Play"
      className={cn(
        "inline-flex h-14 items-center gap-3 rounded-2xl bg-[var(--color-ink)] px-5 text-white transition-transform duration-300 hover:-translate-y-0.5",
        className
      )}
